...
function PricingUI() {
    ...
    const [userId, setUserId] = useState('19'); // Replace with your logic to get the user ID
    ...
    useEffect(() => {
        const fetchSubscription = () => {
            const userSubscribedToPlus = true;
            if (userSubscribedToPlus) {
    ...
    const getSubscription = async () => {
        try {
            const response = await axios.post('http://localhost:8080/api/stripe/create-checkout-session', {
                email,
                productId: 'prod_QDca5iathytx9V' // EduDeck Plus product ID
            });
